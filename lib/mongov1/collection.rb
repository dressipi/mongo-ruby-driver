...
module MongoV1
  ...
  class Collection
    ...
    def initialize(name, db, opts={})
      if db.is_a?(String) && name.is_a?(MongoV1::DB)
      ...
      unless pk_factory
        ...
        MongoV1::ReadPreference::validate(@read)
        @capped             = opts[:capped]
    ...
    def find_and_modify(opts={})
      ...
      cmd[:sort] =
        MongoV1::Support.format_order_clause(opts[:sort]) if opts[:sort]
    ...
    def aggregate(pipeline=nil, opts={})
      ...
      unless MongoV1::Support.ok?(result)
        raise MongoV1::OperationFailure, "aggregate failed: #{result['errmsg']}"
      end
    ...
    def map_reduce(map, reduce, opts={})
      ...
      hash['sort'] = MongoV1::Support.format_order_clause(opts.delete(:sort)) if opts.key?(:sort)

      result = @db.command(hash, command_options(opts))
      unless MongoV1::Support.ok?(result)
        raise MongoV1::OperationFailure, "map-reduce failed: #{result['errmsg']}"
      end
    ...
    def rename(new_name)
      ...
      if new_name.empty? or new_name.include? ".."
        raise MongoV1::InvalidNSName, "collection names cannot be empty"
      end
      if new_name.include? "$"
        raise MongoV1::InvalidNSName, "collection names must not contain '$'"
      end
      if new_name.match(/^\./) or new_name.match(/\.$/)
        raise MongoV1::InvalidNSName, "collection names must not start or end with '.'"
      end
    ...
    def validate_index_types(*types)
      types.flatten!
      types.each do |t|
        unless MongoV1::INDEX_TYPES.values.include?(t)
          raise MongoArgumentError, "Invalid index field #{t.inspect}; " +
                "should be one of " + MongoV1::INDEX_TYPES.map {|k,v| "Mongo::#{k} (#{v})"}.join(', ')
        end
      end
    end
    ...
    def generate_indexes(field_spec, name, opts)
      ...
      begin
        ...
        if MongoV1::ErrorCode::COMMAND_NOT_FOUND_CODES.include?(ex.error_code)
          selector[:ns] = "#{@db.name}.#{@name}"
          send_write(:insert, nil, selector, false, {:w => 1}, MongoV1::DB::SYSTEM_INDEX_COLLECTION)
        else
          raise MongoV1::OperationFailure, "Failed to create index #{selector.inspect} with the following error: " +
           "#{ex.message}"
        end
      end
