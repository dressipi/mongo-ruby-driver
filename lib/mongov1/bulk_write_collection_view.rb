...
module MongoV1
  ...
  class BulkWriteCollectionView
    ...
    def execute(opts = {})
      ...
      raise BulkWriteError.new(MULTIPLE_ERRORS_MSG, MongoV1::ErrorCode::MULTIPLE_ERRORS_OCCURRED, result) if !errors.empty? || !write_concern_errors.empty?
      result
    end
    ...
    def merge_result(errors, exchanges)
      ...
      result = {"ok" => 0, "n" => 0}
      unless errors.empty?
        unless (writeErrors = errors.select { |error| error.class != MongoV1::OperationFailure && error.class != WriteConcernError }).empty? # assignment
        ...
        result.merge!("code" => MongoV1::ErrorCode::MULTIPLE_ERRORS_OCCURRED, "errmsg" => MULTIPLE_ERRORS_MSG)
      end
      exchanges.each do |exchange|
        ...
        if (writeErrors = response["writeErrors"] || response["errDetails"]) # assignment
          ...
          write_concern_error = {"errmsg" => errmsg, "code" => MongoV1::ErrorCode::WRITE_CONCERN_FAILED,
                               "errInfo" => {"wtimeout" => response["wtimeout"]}} # OP_* does not have "code"
        elsif errmsg == "norepl" # OP_*
          write_concern_error = {"errmsg" => errmsg, "code" => MongoV1::ErrorCode::WRITE_CONCERN_FAILED} # OP_* does not have "code"
        ...
        if response["writeConcernError"]
          ...
          write_concern_error = {"errmsg" => wnote, "code" => MongoV1::ErrorCode::WRITE_CONCERN_FAILED} # OP_* does not have "code"
        elsif (jnote = response["jnote"]) # assignment - OP_*
          write_concern_error = {"errmsg" => jnote, "code" => MongoV1::ErrorCode::BAD_VALUE} # OP_* does not have "code"
        end
