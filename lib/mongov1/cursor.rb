...
module MongoV1
  ...
  class Cursor
    ...
    include MongoV1::ReadPreference
    ...
    def count(skip_and_limit = false)
      ...
      return response['n'].to_i if MongoV1::Support.ok?(response)
    ...
    def close
      if @cursor_id && @cursor_id != 0
        ...
        @connection.send_message(
          MongoV1::Constants::OP_KILL_CURSORS,
          message,
          :pool => @pool
        )
      end
    ...
    def inspect
      "<MongoV1::Cursor:0x#{object_id.to_s(16)} namespace='#{full_collection_name}' " +
      ...
      instrument(:find, instrument_payload) do
        begin
          ...
          results, @n_received, @cursor_id = @connection.receive_message(
            MongoV1::Constants::OP_QUERY, message, nil, socket, @command,
          ...
          @connection.refresh
          if tries < 3 && !@socket && (!@command || MongoV1::ReadPreference::secondary_ok?(@selector))
            tries += 1
            retry
          else
            raise ex
    ...
    def send_get_more
      ...
      begin
        results, @n_received, @cursor_id = @connection.receive_message(
          MongoV1::Constants::OP_GET_MORE, message, nil, socket, @command,
          nil, exhaust?, compile_regex?)
      ensure
        socket.checkin
      end
    ...
    def checkout_socket_from_connection
      begin
        if @pool
          socket = @pool.checkout
        elsif @command && !MongoV1::ReadPreference::secondary_ok?(@selector)
    ...
    def construct_query_spec
      ...
      spec['$query']    = @selector
      spec['$orderby']  = MongoV1::Support.format_order_clause(@order) if @order
      ...
      if needs_read_pref?
        read_pref = MongoV1::ReadPreference::mongos(@read, @tag_sets)
