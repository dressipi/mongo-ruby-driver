...
module MongoV1
  ...
  class DB
    ...
    def add_user(username, password=nil, read_only=false, opts={})
      ...
      elsif ex.error_code == MongoV1::ErrorCode::UNAUTHORIZED
    ...
    def remove_user(username)
      begin
        ...
        raise ex unless MongoV1::ErrorCode::COMMAND_NOT_FOUND_CODES.include?(ex.error_code)
    ...
    def collection_names
      if @client.wire_version_feature?(MongoV1::MongoClient::MONGODB_3_0)
    ...
    def command(selector, opts={})
      ...
      command[:limit] = -1
      command[:read] = MongoV1::ReadPreference::cmd_read_pref(opts.delete(:read), selector) if opts.key?(:read)
    ...
    def create_or_update_user(command, username, password, read_only, opts)
      ...
      opts = opts.dup
      pwd = MongoV1::Authentication.hash_password(username, password) if password
    ...
    def legacy_add_user(username, password=nil, read_only=false, opts={})
      ...
      user['pwd'] =
        MongoV1::Authentication.hash_password(username, password) if password
