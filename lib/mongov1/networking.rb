...
module MongoV1
  module Networking
    ...
    def send_message_with_gle(operation, message, db_name, log_message=nil, write_concern=false)
      ...
      last_error_id = add_message_headers(last_error_message, MongoV1::Constants::OP_QUERY)
      ...
      if num_received == 1
        ...
        if error && error.include?("not master")
          ...
          code = docs[0]['code'] || MongoV1::ErrorCode::BAD_VALUE # as of server version 2.5.5
          raise WriteConcernError.new(code.to_s + ': ' + note, code, docs[0])
        elsif error
          code = docs[0]['code'] || MongoV1::ErrorCode::UNKNOWN_ERROR
    ...
    def receive_header(sock, expected_response, exhaust=false)
      ...
      if !exhaust && expected_response != response_to
        raise MongoV1::ConnectionFailure, "Expected response #{expected_response} but got #{response_to}"
      end
    ...
    def check_response_flags(flags)
      if flags & MongoV1::Constants::REPLY_CURSOR_NOT_FOUND != 0
        raise MongoV1::OperationFailure, "Query response returned CURSOR_NOT_FOUND. " +
          "Either an invalid cursor was specified, or the cursor may have timed out on the server."
      elsif flags & MongoV1::Constants::REPLY_QUERY_FAILURE != 0
