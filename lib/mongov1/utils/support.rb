...
module MongoV1
  module Support
    ...
    def validate_db_name(db_name)
      ...
      [" ", ".", "$", "/", "\\"].each do |invalid_char|
        if db_name.include? invalid_char
          raise MongoV1::InvalidNSName, "database names cannot contain the character '#{invalid_char}'"
        end
      end
      raise MongoV1::InvalidNSName, "database name cannot be the empty string" if db_name.empty?
      db_name
    end

    def format_order_clause(order)
      case order
        when Hash, BSONV1::OrderedHash then hash_as_sort_parameters(order)
    ...
    def normalize_seeds(seeds)
      pairs = Array(seeds)
      pairs = [ seeds ] if pairs.last.is_a?(Integer)
